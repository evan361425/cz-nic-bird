...
typedef struct net_addr_evpn_eth {
  u8 type;
  u8 subtype;
  u16 length;
  u32 tag;
...
#define NET_ADDR_EVPN_ETH(rd, tag, esi) \
  ((net_addr_evpn_eth) { NET_EVPN, NET_EVPN_ETH, sizeof(net_addr_evpn_eth), tag, rd, .esi = esi })
...
static inline void net_fill_evpn_eth(net_addr *a, u64 rd, u32 tag, evpn_esi esi)
{ *(net_addr_evpn_eth *)a = NET_ADDR_EVPN_ETH(rd, tag, esi); }
